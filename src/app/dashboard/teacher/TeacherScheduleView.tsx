...
export default function TeacherScheduleView({ tuitions }: { tuitions: Tuition[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="glass-card rounded-2xl overflow-hidden shadow-xs">
        <div className="flex items-center gap-3 p-5 sm:p-6 border-b border-slate-200/80 bg-white/60">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">{tuitions.length} active {tuitions.length === 1 ? "enrolment" : "enrolments"} — click a student to view their schedule</p>
          </div>
        </div>
              ...
              return (
                <li key={t.id}>
                  <button
                    ...
                    className={`w-full text-left p-4 border rounded-xl transition-all flex items-center gap-3 ${
                      isSelected
                        ? "border-blue-400 bg-blue-50 shadow-md shadow-blue-100"
                        : "border-slate-200/80 bg-white/90 hover:border-blue-300 hover:shadow-sm"
                    }`}
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-extrabold text-sm shrink-0 transition-all ${
                      isSelected
                        ? "bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-md shadow-blue-300"
                        : "bg-gradient-to-br from-blue-100 to-indigo-100 border border-blue-200 text-blue-700"
                    }`}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex gap-1.5 mt-1.5 flex-wrap">
                        <span className="text-[10px] font-bold text-emerald-600">{s.attended}✓</span>
                        <span className="text-[10px] font-bold text-red-500">{s.missed}✗</span>
                        <span className="text-[10px] font-bold text-blue-500">{s.upcoming} upcoming</span>
                      </div>
                    </div>
                    {isSelected && (
      ...
      {/* Schedule Panel for Selected Student */}
      {selected && (
        <section className="glass-card rounded-2xl overflow-hidden shadow-xs">
          {/* Header */}
          <div className="p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 bg-white/60">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-extrabold text-slate-900 text-lg flex items-center gap-2">
                  ...
                  {selected.grade && (
                    <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">{selected.grade}</span>
                  )}
                </h3>
            ...
            <div className="flex gap-2 flex-wrap">
              {(() => { const s = stats(selected.sessions); return (
                <>
                  <span className="px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-xl text-xs font-bold text-emerald-700">{s.attended} Completed</span>
                  <span className="px-3 py-1.5 bg-red-50 border border-red-200 rounded-xl text-xs font-bold text-red-700">{s.missed} Missed</span>
                  <span className="px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-xl text-xs font-bold text-blue-700">{s.upcoming} Upcoming</span>
                </>
              ); })()}
            </div>
          </div>
          ...
          <div className="p-5 sm:p-6 space-y-6">
            ...
            {selected.sessions.length === 0 ? (
              <p className="text-sm text-slate-500 py-6 text-center">No sessions scheduled for this student yet.</p>
            ) : (
              <div className="border border-slate-200/80 rounded-xl overflow-auto bg-white/90 shadow-xs max-h-[520px]">
