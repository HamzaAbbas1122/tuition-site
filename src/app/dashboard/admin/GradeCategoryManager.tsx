        ...
        <div className="flex flex-col gap-4">
          {filteredStudents.length === 0 ? (
            ...
            filteredStudents.map((s) => (
              <div key={s.id} className="p-5 rounded-2xl bg-white/90 border border-slate-200/80 shadow-xs hover:shadow-md hover:border-blue-300 transition-all duration-300 flex flex-col xl:flex-row gap-5 items-start xl:items-center justify-between">
                
                {/* 1. Basic Info */}
                <div className="flex items-center space-x-4 min-w-[220px]">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white font-extrabold flex items-center justify-center text-lg shadow-xs shrink-0">
                ...
                <div className="flex-1 w-full xl:w-auto xl:border-l xl:border-slate-100 xl:pl-6 py-2 xl:py-0 border-y xl:border-y-0 border-slate-100 my-2 xl:my-0">
                  <div className="space-y-2">
                    <p className="text-[11px] font-extrabold text-slate-500 uppercase tracking-wider flex items-center justify-between xl:justify-start xl:gap-3">
                      ...
                      <span className="text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">{s.studentClasses.length} Class{s.studentClasses.length !== 1 ? "es" : ""}</span>
                    </p>
                    {s.studentClasses.length === 0 ? (
                      ...
                      <ul className="flex flex-wrap gap-2 max-h-24 overflow-y-auto">
                        {s.studentClasses.map((c) => (
                          <li key={c.id} className="text-xs text-slate-800 bg-slate-50/80 px-3 py-1.5 rounded-xl border border-slate-200/70 shadow-xs inline-flex items-center gap-1.5">
                            <span className="font-bold text-blue-700">{c.subject}</span>
                            <span className="text-slate-300">|</span>
                            <span className="font-medium text-slate-600 truncate max-w-[120px]">{c.teacher.name}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                ...
                <div className="w-full xl:w-auto xl:border-l xl:border-slate-100 xl:pl-6 shrink-0">
                  {editingStudentId === s.id ? (
                    <form action={async (formData) => {
                      ...
                      <span className="text-xs font-bold text-slate-600 xl:hidden">Assign Class:</span>
                      <div className="flex items-center gap-1.5 w-full sm:w-auto">
                        <select name="grade" defaultValue={s.studentProfile?.grade || "Class 9"} className="glass-input text-xs rounded-xl px-2.5 py-2 bg-white font-medium border border-slate-200 flex-1 sm:flex-none">
                        ...
                        <button type="button" onClick={() => setEditingStudentId(null)} className="text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-600 px-2.5 py-2 rounded-xl transition-all shrink-0" title="Cancel">
                          ✕
                        </button>
                      </div>
                    </form>
                    ...
                    <div className="flex flex-row xl:flex-col items-center xl:items-end justify-between gap-3">
                      <div className="flex items-center gap-1.5 text-xs text-slate-600">
                        <span className="font-semibold text-slate-500 xl:hidden">Class:</span>
                        <span className="font-extrabold text-blue-700 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200 shadow-xs">{s.studentProfile?.grade || "Class 9"}</span>
                      </div>
                      <button 
